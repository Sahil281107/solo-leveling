...
export const getAllQuestsDetailed = async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      if (status === 'active') {
        whereClause = 'WHERE aq.is_completed = FALSE';
        countWhereClause = 'WHERE is_completed = FALSE';
      } else if (status === 'completed') {
        whereClause = 'WHERE aq.is_completed = TRUE';
      ...
      const [quests]: any = await connection.execute(`
        ...
        LEFT JOIN users u ON aq.user_id = u.user_id
        ${whereClause}
      ...
      const [countResult]: any = await connection.execute(`
        SELECT COUNT(*) as total FROM active_quests
        ${countWhereClause}
      `);
      ...
      connection.release();
      console.error('Quest query error:', queryError);
      res.json({
        quests: [],
