...
export const getDashboardStats = async (req: Request, res: Response) => {
  try {
    ...
    // Try to get quest count, but don't fail if table doesn't exist
    let totalQuests = 0;
    try {
      const questResult = await prisma.$queryRaw`SELECT COUNT(*) as count FROM active_quests`;
      totalQuests = (questResult as any)[0]?.count || 0;
    } catch (questError) {
      console.warn('⚠️ active_quests table not found, returning 0 quests');
      totalQuests = 0;
    }
